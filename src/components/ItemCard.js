...
const ItemCard = ({ item, resInfo }) => {
    const cartItem = useSelector(store => store.cart.items.filter(i => i.id === item.id))
    const dispatch = useDispatch()

    const handleItemAddClick = () => {
        if(cartItem.length > 0) {
            if(cartItem.resInfo.id === resInfo.id) {
                ...
                <div>
                    ...
                    {isInCart ? 
                        <div className='flex absolute gap-0 mt-[-2rem] ml-6'>
                            ...
                            <div className='py-1 px-2  bg-white  border-t-2 border-b-2 border-orange-500  font-bold'>{cartItem.amount}</div>
                            <button 
